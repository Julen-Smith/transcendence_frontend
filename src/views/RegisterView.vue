<template>
  <div v-if="isLoading"><LoadingScreen/></div>
  <div v-else class="video-container">
    <video autoplay muted loop playsinline class="background-video">
      <source src="../assets/video/Astro.webm" type="video/webm" />
    </video>
  <div class="register-container content-overlay" v-if="!showOTPVerification">
    
    <div class="register-form container-gradient">
    <h2 class="Cybertitle">{{ t('message.register') }}</h2>
      <form @submit.prevent="checkRegister">
        ...
        <div class="form-group flex flex-col">
          ...
          <input class="input" type="email" id="email" v-model="credentials.email" autocomplete="email">
        </div>
        <div class="form-group flex flex-col">
          <label for="password">{{ t('message.password') }}:</label>
          <input class="input" type="password" id="password" v-model="credentials.password" autocomplete="new-password">
          <p v-if="passwordError" class="error-message">{{ passwordError }}</p>
        </div>
        <div class="form-group flex flex-col">
          <label for="confirmPassword">{{ t('message.confirm_pass') }}:</label>
          <input class="input" type="password" id="confirmPassword" v-model="confirmPassword" autocomplete="new-password">
        </div>
        <div class="form-group flex flex-col">
        <button class="button" type="submit" :disabled="!isFormValid">{{ t('message.register') }}</button>
        </div>
      </form>
      ...
      <div class="form-group flex flex-col">
        <GoogleLogin :buttonText="googleButtonText" ></GoogleLogin>
      </div>
    </div>
  </div>
</div>
...
import { check_register, handleIntraRedirect } from '@/methods/api/login.js';
import { useI18n } from 'vue-i18n'; 

export default {
  name: 'RegisterView',
  components: {PopUpError, GoogleLogin, OTPVerification, LoadingScreen},
  setup() {
    ...
    const validateUsername = (username) => {
      ...
      if (username.length > 15) {
        usernameError.value = 'El nombre de usuario no debe exceder los 15 caracteres';
        return false;
      }
    ...
    const validatePassword = (password, username) => {
      ...
      if (password.length > 32) {
        passwordError.value = 'La contraseña no debe exceder los 32 caracteres';
        return false;
      }
      ...
      if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
        passwordError.value = 'La contraseña debe contener al menos un símbolo';
        return false;
      }
      if (username && password.includes(username)) {
        passwordError.value = 'La contraseña no puede contener el nombre de usuario';
        return false;
      }
      passwordError.value = '';
      return true;
    ...
    const isFormValid = computed(() => {
      return credentials.value.password === confirmPassword.value && 
             ...
             validateUsername(credentials.value.username) &&
             validatePassword(credentials.value.password, credentials.value.username);
    });
    ...
    const checkRegister = async () => {
      try {
        const { success, error } = await check_register(credentials.value);
        if (success) {
        ...
        errorMessage.value = t('message.err_register');
        popupTriggers.value.responseTrigger = true;
      }
    };
